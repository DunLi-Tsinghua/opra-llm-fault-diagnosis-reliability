...
def make_numeric_figures(counts: np.ndarray) -> dict[str, float]:
    ...
    ax.plot(n_values, lower_profile, color="#1F77B4", linewidth=1.8, label="Lower bound")
    ax.plot(n_values, upper_profile, color="#1F77B4", linewidth=1.8, linestyle="--", label="Upper bound")
    ax.axhline(R_REQ, color="#8B1A1A", linewidth=1.4, linestyle=":", label="Requirement")
    ax.set_xlabel("Future diagnosis tasks, $n$")
    ax.set_ylabel("Reliability")
    ax.set_ylim(0, 1.02)
    ax.grid(True, alpha=0.25)
    ...
    ax.set_ylabel(f"Lower reliability envelope at $n={MISSION_N_PLANNING}$")
